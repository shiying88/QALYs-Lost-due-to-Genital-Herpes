...
class DecisionNode(Node):
    ...
    def get_component_loss(self):
        """ return breakdowns of the expected cost and health utility of each decision """
        outcomes = dict()
        for node in self.futureNodes:
            outcomes[node.name] = node.evaluate_by_component()
...
def _aggregate_dicts(current_dict, new_dict):
    for key in new_dict:
        if key in current_dict:
            current_dict[key] = current_dict[key] + new_dict[key]
        else:
...
class ChanceNode(Node):
    def __init__(self, name, dict_chances, dict_terminals):
        ...
        if name in dict_chances:
            ...
            self.component_dic = dict_chances[name][Columns.DICS.value]   # find component utility
            assert sum(self.component_dic.values()) == self.utility
    ...
    def evaluate_by_component(self):
        """ add utility loss from recurrent components """
        current_dict = {}
        i = 0  # iterator in future nodes
        for node in self.futureNodes:
            sub_result = node.evaluate_by_component()
            prob = self.pFutureNodes[i]
            for key in sub_result:
                sub_result[key] = sub_result[key] * prob
            _aggregate_dicts(current_dict, sub_result)
            i += 1
        _aggregate_dicts(current_dict, self.component_dic)
        return current_dict
...
class TerminalNode(Node):
    def __init__(self, name, dict_terminals):
        ...
        if name in dict_terminals:
            ...
            # find component disutility dictionary
            self.component_dic = copy.deepcopy(dict_terminals[name][Columns.DICS.value])
    ...
    def evaluate_by_component(self):
        """ add utility loss from recurrent components """
        return copy.deepcopy(self.component_dic)
...
def convert_node_name_to_event_name(name_to_event_map, node_name):
    if node_name not in name_to_event_map:
        return 'default'
    return name_to_event_map[node_name]
